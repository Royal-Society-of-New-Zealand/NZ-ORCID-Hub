...
EmpRecord = namedtuple("EmpRecord", [
    "name", "city", "state", "country", "department", "role", "start_date",
    "end_date"
])
...
def year_range(entry):
    ...
    if entry.get("start_date"
                 ) is None or entry["start_date"]["year"]["value"] is None:
        val = "unknown"
    else:
    ...
    if entry.get(
            "end_date") is None or entry["end_date"]["year"]["value"] is None:
        val += "present"
    else:
...
def delete_employment(user_id, put_code=None):
    ...
    try:
        user = User.get(
            id=user_id, organisation_id=current_user.organisation_id)
    except:
        flash("ORCID HUB doent have data related to this researcher",
              "warning")
...
def employment(user_id, put_code=None):
    ...
    _url = request.args.get('url') or url_for(
        "employment_list", user_id=user_id)
    ...
    if put_code is not None:
        try:
            ...
            data = EmpRecord(
                ...
                state=_data.get("organization").get("address").get(
                    "region", ""),
                country=_data.get("organization").get("address").get(
                    "country", ""),
            ...
            print(
                "Exception when calling MemberAPIV20Api->view_employment: %s\n"
                % e.body)
    else:
        data = None
    ...
    if request.method == "POST" and form.validate():
        ...
        try:
            if put_code:
                api_instance.update_employment(
                    user.orcid, put_code, body=employment)
            else:
                api_response = api_instance.create_employment(
                    user.orcid, body=employment)
            ...
            if put_code:
                ...
                flash("Employment Details has been added successfully!",
                      "success")
            return redirect(_url)
        except ApiException as e:
...
def employment_list(user_id):
    ...
    try:
        ...
        flash("User didnt gave permission to update his/her records",
              "warning")
        return redirect(url_for("viewmembers"))
